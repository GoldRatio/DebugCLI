...
from harness.operator.router import _keyword_route, route_message
...
class _ScriptedReader:
    ...
    def __init__(self, lines, yield_when=None):
...
def test_router_uses_llm():
    llm = _RouterLLM({"intent": "diagnose", "symptom": "ECC errors", "host": "h1"})
    cmd = route_message("ECC errors", llm, host_names=("h1", "h2"))
    assert cmd.intent == "diagnose"
    assert cmd.symptom == "ECC errors"
    assert cmd.host == "h1"
    assert llm.messages[0]["role"] == "system"


def test_router_unknown_host_falls_back_to_active():
    llm = _RouterLLM({"intent": "diagnose", "symptom": "x", "host": "nope"})
    cmd = route_message("x", llm, host_names=("h1",))
    assert cmd.host is None


def test_router_malformed_llm_output_falls_back():
    llm = _RouterLLM({"kind": "question", "question": "?"})
    cmd = route_message("ECC errors again", llm)
    assert cmd.intent == "diagnose"
    assert cmd.symptom == "ECC errors again"
...
def test_router_runs_in_background_and_announces_launch(tmp_path, capsys):
    """A slow router LLM must NOT freeze the REPL: the operator keeps typing
    (lines queue up), a status tick shows the agent working in the background,
    and the routed action is announced and runs once routing completes.
    ...
    threading.Timer(1.0, gate.set).start()  # release routing shortly after start

    class BlockingRouter:
        def chat_json(self, messages):
            gate.wait(5.0)
            return {"intent": "reply", "text": "hi"}
    ...
    class Reader(_ScriptedReader):
        ...
        def poll(self, timeout):
            ...
            if task is not None and task.alive and not self.queued:
                self.queued = True
                return "second line while routing"
            return None

    def on_session(session):
        session.last_tick = time.monotonic() - 60.0  # tick fires on the 1st busy poll
        holder["s"] = session
    ...
    code = run_session(_session_args(tmp_path), overrides={
        ...
        "on_session": on_session,
        "llm": StubLLM(),
        "router_llm": BlockingRouter(),
    })
    assert code == 0

    out = capsys.readouterr().out
    assert "thinking: routing your message to the agent" in out
    assert "running in the background: routing to agent" in out
    assert "started in the background: reply" in out
    assert "done: (no reply)" in out

    session = holder["s"]
    assert session.pending == ["second line while routing"]


def test_session_shows_background_status_while_busy(tmp_path, capsys):
    """While a diagnose runs, the REPL keeps printing a status line so the
    operator can see the agent is working in the background."""
    holder = {}

    def on_session(session):
        session.last_tick = time.monotonic() - 60.0
        holder["s"] = session
